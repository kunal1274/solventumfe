...
function App3plSoObVsIb() {
  ...
  const [progressMessage, setProgressMessage] = useState('');
   // ▶ ADDING NEW STATE FOR WRAP CONTROL
   const [wrapLogs, setWrapLogs] = useState(false); // False means no wrap initially
  ...
  const doComparison = () => {
    ...
    // Create a map for inbound by keyField
    const inboundMap = {};
    ...
    const results = outboundData.map(outItem => {
      ...
      const inboundState = inItem ? inItem.finalState : 'F'; // If missing inbound, consider fail or handle differently
      ...
      // ADDED: Extract logDescription fields
  ...
  const inboundLogDescription = inItem ? (inItem.logDescription || '') : '';
      ...
      return {
        ...
        inboundSuccessCount,
        inboundFailureCount,
        outboundLogDescription, // ADDED: Include outboundLogDescription
        inboundLogDescription   // ADDED: Include inboundLogDescription
      };
    });
  ...
  return (
    <div>
      ...
      onClick={doComparison} disabled={outboundData.length === 0 || inboundData.length === 0}>
        Compare
      </button>

 {/* ▶ NEW BUTTON TO TOGGLE WRAP */}
          ...
          <tbody>
            {comparisonResults.map((r, i) => (
              <tr key={i}>
                ...
                <td>
                  {/* ▶ USING PRE TAG OR DIV WITH WRAP STYLES */}
                  <div style={{
